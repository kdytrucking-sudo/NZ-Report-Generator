...
export default function SingleChoiceSettingsPage() {
    ...
    const [cards, setCards] = useState<SingleChoiceCard[]>([]);
    ...
    const loadCards = async (uid: string) => {
        setLoading(true);
        const data = await getSingleChoiceCards(uid);
        setCards(data);
        setLoading(false);
    };

    const handleAddCard = () => {
        const newCard: SingleChoiceCard = {
        ...
        // Add to local state (edit mode immediately basically)
    ...
    const handleCardChange = (index: number, field: keyof SingleChoiceCard, value: any) => {
        const newCards = [...cards];
        newCards[index] = { ...newCards[index], [field]: value };
        setCards(newCards);
        // Auto-save logic could go here, or manual save button per card
    };
    ...
    const handleSaveCard = async (index: number) => {
        ...
        try {
            const newId = await saveSingleChoiceCard(user.uid, card);
            if (newId) {
                // Update local state id in case it was new
                const newCards = [...cards];
    ...
    const handleOptionChange = (cardIndex: number, optionIndex: number, field: keyof SingleChoiceOption, value: string) => {
        const newCards = [...cards];
