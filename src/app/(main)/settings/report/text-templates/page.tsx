...
import { getTextTemplateCards, saveTextTemplateCard, deleteTextTemplateCard, TextTemplateCard, TextTemplateOption } from "@/lib/firestore-text-templates";
...
export default function TextTemplatesPage() {
    ...
    const [cards, setCards] = useState<TextTemplateCard[]>([]);
    ...
    const loadCards = async (uid: string) => {
        setLoading(true);
        const data = await getTextTemplateCards(uid);
        setCards(data);
        setLoading(false);
    };

    const handleAddCard = () => {
        const newCard: TextTemplateCard = {
            id: "", // Will be generated on save
            uid: user.uid,
            name: "New Text Template",
            placeholder: "[Replace_TextTemplate]",
            options: [
                { id: '1', label: '', value: '' },
                { id: '2', label: '', value: '' },
                { id: '3', label: '', value: '' }
    ...
    const handleDeleteCard = async (index: number) => {
        ...
        if (confirmed) {
            if (card.id) {
                await deleteTextTemplateCard(user.uid, card.id);
            }
    ...
    const handleCardChange = (index: number, field: keyof TextTemplateCard, value: any) => {
        const newCards = [...cards];
    ...
    const handleSaveCard = async (index: number) => {
        ...
        try {
            const newId = await saveTextTemplateCard(user.uid, card);
            if (newId) {
                const newCards = [...cards];
    ...
    const handleAddOption = (cardIndex: number) => {
        const newCards = [...cards];
        const newOption: TextTemplateOption = {
    ...
    const handleOptionChange = (cardIndex: number, optionIndex: number, field: keyof TextTemplateOption, value: string) => {
        const newCards = [...cards];
        ...
        <>
            ...
            <div className={styles.container}>
                <div className={styles.header}>
                    <div>
                        <h1 className={styles.title}>Manage Text Templates</h1>
                        <p className={styles.description}>Create and manage multi-line text templates for report generation.</p>
                    </div>
                            ...
                            <div className={styles.cardHeader}>
                                <div className={styles.field}>
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. Standard Clauses"
                                    />
                                </div>
                            ...
                            <div className={styles.cardBody}>
                                <div className={styles.columnsContainer}>
                                    ...
                                    <div className={`${styles.column} ${styles.columnRight}`}>
                                        <div className={styles.columnHeader}>Text Content</div>
                                        <div className={styles.inputStack}>
                                            {card.options.map((option, optionIndex) => (
